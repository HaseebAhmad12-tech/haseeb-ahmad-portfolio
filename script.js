...
const syncCommunityBlogs = async () => {
  ...
  try {
    ...
    if (blogSyncStatus) blogSyncStatus.textContent = 'Blog cards are synced from my public Power Platform Community profile and new public posts are picked up automatically.';
    blogPage = 0;
    renderBlogCarousel();
  } catch (error) {
    if (blogCount) blogCount.textContent = String(blogTrack.children.length);
